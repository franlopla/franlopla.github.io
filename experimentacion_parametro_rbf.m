...
% Numero de condicion
cond_A_gaussiana_2=cond(A);

% Aplicamos el interpolante visto en teoria
r_eval=sqrt((X_malla(:)-X_datos').^2+(Y_malla(:)-Y_datos').^2);
Z_interp=rbf(r_eval)*lambda;
Z_interp=reshape(Z_interp,size(X_malla));
error_gaussiana_2=Z_exacta-Z_interp;   
ecm_gaussiana_2=sqrt(mean(error_gaussiana_2(:).^2));
emax_gaussiana_2=max(abs(error_gaussiana_2(:)));

% Generamos la grafica marcando los nodos que queremos interpolar en rojo
% para comprobar que la superficie interpola a los nodos
figure('Name','Interpolacion rbf(gaussiana), epsilon=0.1');
hold on;
grid on;
surf(X_malla,Y_malla,Z_interp,'EdgeColor','none','FaceAlpha',0.8);
scatter3(X_datos,Y_datos,Z_datos,'MarkerEdgeColor','k','MarkerFaceColor','red');
...
figure('Name','Interpolacion rbf(multicuadrica) epsilon=15');
hold on;
grid on;
surf(X_malla,Y_malla,Z_interp,'EdgeColor','none','FaceAlpha',0.8);
scatter3(X_datos,Y_datos,Z_datos,'MarkerEdgeColor','k','MarkerFaceColor','red');
...
cond_A_multicuadrica_2=cond(A);

r_eval=sqrt((X_malla(:)-X_datos').^2+(Y_malla(:)-Y_datos').^2);
Z_interp=rbf(r_eval)*lambda;
Z_interp=reshape(Z_interp,size(X_malla));
error_multicuadrica_2=Z_exacta-Z_interp;   
ecm_multicuadrica_2=sqrt(mean(error_multicuadrica_2(:).^2));
emax_multicuadrica_2=max(abs(error_multicuadrica_2(:)));

figure('Name','Interpolacion rbf(multicuadrica), epsilon=0.1');
hold on;
grid on;
surf(X_malla,Y_malla,Z_interp,'EdgeColor','none','FaceAlpha',0.8);
scatter3(X_datos,Y_datos,Z_datos,'MarkerEdgeColor','k','MarkerFaceColor','red');
...
% radial que vamos a utilizar
epsilon=1; 
rbf=@(r)sqrt(r.^2+epsilon^2); % Función Gaussiana
